...
public class RFX1_TransformMotion : Photon.MonoBehaviour
{
    ...
    void OnCollisionBehaviour(RaycastHit hit)
    {
		...
		if(hit.transform.root.GetComponent<PhotonView> () && photonView.isMine){
		hit.transform.root.GetComponent<PhotonView> ().RPC ("ApplyDamage", PhotonTargets.AllBuffered, Damdge, playerName );
				Debug.Log("i hit you " + hit.transform.name + " ");
